...
registerFragment('sidebar').then(({ parent, config }: { parent: HTMLElement, config: { universe: string, baseUrl: string, routes: any[] } }) => {
  ...
  import(`./translations/Messages_${locale}.json`)
    ...
    .then((translations) => {
      ...
      return getItems(config.universe, universeRoutes)
        .then((sidebarConfig) => {
          return new Vue({
            i18n,
